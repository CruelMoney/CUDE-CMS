...
class EditableImage extends React.Component {
    ...
    upload = (theFile) => {
        ...
        const dbImage = Array.isArray(this.props.data) && this
            .props
            .data
            .find(img => img.name === this.props.dbKey)
        const url = dbImage
            ? dbImage.url
            : null
        const endPoint = dbImage
            ? `/api/fileupload/${dbImage._id}/update`
            : '/api/fileupload/create'

        return () => new Promise((resolve, reject) => {
            setTimeout(() => {

                fetch(Config.PUBLIC_URL + endPoint, {
                    method: dbImage
                        ? "PUT"
                        : 'POST',
                    credentials: 'include',
    ...
    handleAccept = (files) => {
        ...
        this
            .props
            .registerPromise(this.upload(files[0]))
    }
    ...
    render() {
        ...
        return (this.props.editMode
            ? <div className={styles.wrapper + " editable"}>
                    <Dropzone maxSize={4000000} //byte
                        multiple={false} className={styles.dropZone} accept="image/jpeg, image/png" onDrop={(accepted, rejected) => {
                        this.setState({accepted, rejected});
                    }} onDropRejected={(file) => this.handleReject(file)} onDropAccepted={(file) => this.handleAccept(file)} activeStyle={{
                    ...
                    {url
                        ? <LoadingImage
                                {...this.props}
                                src={this.state.src || (Config.PUBLIC_URL + url)}
                                alt=""/>
                        : null}

                </div>
            : (url
                ? <LoadingImage {...this.props} src={Config.PUBLIC_URL + url} alt=""/>
                : null));
    }
}

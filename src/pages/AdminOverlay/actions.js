...
export function saveEdits(apiData) {
  return (dispatch, getState) => {
    ...
    Object
      .keys(apiData)
      .forEach(endpoint => {
        if (!apiData[endpoint].isEndpoint || !apiData[endpoint].edits) {
          return
        }
        Object
          .keys(apiData[endpoint].edits)
          .forEach(id => {
            promises.push(fetch(Config + endpoint + '/' + id, {
              method: 'PATCH',
              credentials: 'include',
    ...
    // Add other promises, for example image uploads apidata.promises, is a list of
